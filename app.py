...
#     else:
#         if not (0.0 <= overs <= 20.0 and overs % 0.1 == 0):
#             st.error("Please enter a valid value for Overs Bowled (e.g., 0.0, 0.1, ..., 20.0).")
#         else:
...
# Main Page Inputs
st.header('Match Details')
selected_venue = st.selectbox('Select City where match is played', sorted(cities))
Target = st.number_input('Target to Chase',min_value=0, step=1)
